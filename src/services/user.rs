//! 用户业务层 —— 对应 .NET `UserClient` + `RawUserApi`。13 个端点。
//!
//! 大部分是带 session 凭证的 Default 签名代理。user/detail、user/listen、user/follow、
//! user/cloud 用 RSA 加密 p 参数（裸模幂 / PKCS1）。所有需登录的端点调用前应校验登录态。
...
/// user/detail —— 用户详情（裸 RSA p 参数）。
pub async fn user_detail(state: &AppState, session: &KgSession) -> AppResult<Value> {
...
/// user/vip/detail —— VIP 信息（Default 签名，无加密）。
pub async fn user_vip_detail(state: &AppState, session: &KgSession) -> AppResult<Value> {
...
/// user/playlist —— 用户歌单（Default 签名）。
pub async fn user_playlist(state: &AppState, session: &KgSession, page: i64, pagesize: i64) -> AppResult<Value> {
    require_login(session)?;
    let body = json!({
...
/// user/history —— 听歌历史（Default 签名）。
pub async fn user_history(state: &AppState, session: &KgSession) -> AppResult<Value> {
    require_login(session)?;
    let body = json!({
...
/// user/listen —— 听歌排行（裸 RSA p 参数）。
pub async fn user_listen(state: &AppState, session: &KgSession, list_type: i64) -> AppResult<Value> {
    require_login(session)?;
    let client_time = chrono::Utc::now().timestamp();
...
/// user/follow —— 关注的歌手（裸 RSA p 参数）。
pub async fn user_follow(state: &AppState, session: &KgSession) -> AppResult<Value> {
    require_login(session)?;
    let client_time = chrono::Utc::now().timestamp();
    let pk_data = json!({ "clienttime": client_time, "token": session.token });
    let p = crypto::rsa_encrypt_no_padding(&pk_data.to_string(), true).to_uppercase();
...
/// user/cloud —— 云盘（AES-128 + RSA PKCS1，clear_default_params + not_signature）。
pub async fn user_cloud(state: &AppState, session: &KgSession, page: i64, pagesize: i64) -> AppResult<Value> {
    require_login(session)?;
    let client_time = chrono::Utc::now().timestamp();
    ...
    // binary body = base64 解码后的 AES 密文
    let bin = base64::engine::general_purpose::STANDARD
        .decode(&aes.cipher_text)
        .map_err(|e| AppError::Internal(anyhow::anyhow!("云盘 body 解码失败: {e}")))?;
    ...
    if let Some(raw) = resp.get("__raw_base64__").and_then(|v| v.as_str()).filter(|s| !s.is_empty())
        && let Ok(dec) = serde_json::from_str::<Value>(&crypto::playlist_aes_decrypt(raw, &aes.temp_key))
    {
        return Ok(dec);
    }
    Ok(resp)
...
/// user/cloud/url —— 云盘播放链接（Default 签名，calc_cloud_key）。
...
/// user/follow/message —— 关注歌手动态（Default 签名）。
pub async fn user_follow_message(state: &AppState, session: &KgSession, artist_id: &str, pagesize: i64) -> AppResult<Value> {
    require_login(session)?;
...
/// user/video/collect —— 收藏的视频（Default 签名）。
pub async fn user_video_collect(state: &AppState, session: &KgSession, page: i64, pagesize: i64) -> AppResult<Value> {
    require_login(session)?;
...
/// user/video/love —— 点赞的视频（Default 签名）。
pub async fn user_video_love(state: &AppState, session: &KgSession, pagesize: i64) -> AppResult<Value> {
...
/// favorite/count —— 收藏数（Default 签名）。
pub async fn favorite_count(state: &AppState, session: &KgSession, mixsongids: &str) -> AppResult<Value> {
...
/// server/now —— 服务器时间（Default 签名）。
pub async fn server_now(state: &AppState, session: &KgSession) -> AppResult<Value> {
