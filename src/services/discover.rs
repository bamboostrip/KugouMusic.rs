//! 发现/推荐业务层 —— 对应 .NET `RecommendClient` + `RawDiscoveryApi`。
//!
//! 软依赖会话：多数端点用 userid/viptype/mid 但不强制校验，匿名也能跑。
...
/// top/playlist —— 推荐歌单（body 内嵌 key/mid，Default 签名）。
pub async fn recommend_playlists(
    state: &AppState, session: &KgSession, category_id: i64, page: i64,
) -> AppResult<Value> {
...
/// top/song —— 新歌速递（Default 签名）。
pub async fn new_songs(state: &AppState, session: &KgSession, rank_id: i64, page: i64) -> AppResult<Value> {
    let body = json!({
...
/// recommend/songs —— 每日推荐（软依赖 userid，Default 签名）。
pub async fn recommend_songs(state: &AppState, session: &KgSession) -> AppResult<Value> {
...
/// everyday/style/recommend —— 每日风格推荐（Default 签名）。
pub async fn recommend_style(state: &AppState, session: &KgSession) -> AppResult<Value> {
...
/// ai/recommend —— AI 推荐（clear_default_params，body 内嵌 key，Default 签名）。
pub async fn ai_recommend(state: &AppState, session: &KgSession, album_audio_ids: &str) -> AppResult<Value> {
...
/// yueku —— 乐库（Default 签名）。
pub async fn yueku(state: &AppState, session: &KgSession) -> AppResult<Value> {
...
/// yueku/banner —— 乐库 banner（Default 签名）。
pub async fn yueku_banner(state: &AppState, session: &KgSession) -> AppResult<Value> {
    let body = json!({
...
/// yueku/fm —— 乐库电台（Default 签名）。
pub async fn yueku_fm(state: &AppState, session: &KgSession) -> AppResult<Value> {
...
/// top/album —— 新碟上架（Default 签名）。
pub async fn top_album(state: &AppState, session: &KgSession, page: i64, pagesize: i64) -> AppResult<Value> {
    let body = json!({
...
/// top/card —— 卡片歌曲推荐（body 内嵌 key，Default 签名）。
pub async fn top_card(state: &AppState, session: &KgSession, card_id: i64) -> AppResult<Value> {
    let client_time_ms = chrono::Utc::now().timestamp_millis();
    let body = json!({
...
/// top/ip —— 编辑精选（Default 签名）。注：.NET 有 AddIpIdFromInnerUrl 后处理，这里透传。
pub async fn top_ip(state: &AppState, session: &KgSession) -> AppResult<Value> {
...
/// pc/diantai —— banner（Default 签名）。
pub async fn pc_diantai(state: &AppState, session: &KgSession) -> AppResult<Value> {
...
/// brush —— 刷刷推荐（Default 签名）。
pub async fn brush(state: &AppState, session: &KgSession, song_pool_id: i64, mode: &str) -> AppResult<Value> {
    let client_time_ms = chrono::Utc::now().timestamp_millis();
    let pr = json!({
...
/// everyday/history —— 历史推荐（POST，Default 签名）。
...
/// personal/fm —— 私人 FM 推荐/行为上报（Default 签名）。
