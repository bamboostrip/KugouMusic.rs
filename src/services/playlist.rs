//! 歌单写操作业务层 —— 对应 .NET `PlaylistClient` 写类 + `RawPlaylistApi` 写方法。
//!
//! 端点：playlist/add（收藏/创建）、playlist/create（=collect source=1）、
...
/// playlist/add —— 收藏歌单。对应 RawPlaylistApi.CollectPlaylistAsync（Default 签名，无加密）。
...
/// playlist/create —— 新建歌单（= collect，source=1, is_pri=type）。
...
/// playlist/del —— 删除歌单（AES-128 + RSA PKCS1，响应也加密）。
pub async fn delete_playlist(
    ...
    if let Some(enc) = encrypted.filter(|s| !s.is_empty())
        && let Ok(dec) = serde_json::from_str::<Value>(&crypto::playlist_aes_decrypt(enc, &aes.temp_key))
    {
        return Ok(dec);
    }
    Ok(resp)
...
/// playlist/tracks/add —— 添加歌曲到歌单（Default 签名，无加密）。
...
/// playlist/tracks/del —— 从歌单删除歌曲（Default 签名，无加密）。
...
// ===== 歌单读类端点（对应 RawPlaylistApi 读方法，Phase 3）=====

/// sheet/collection —— 曲谱合集（Web 签名）。
pub async fn sheet_collection(state: &AppState, session: &KgSession, position: i64) -> AppResult<Value> {
...
/// sheet/collection/detail —— 曲谱合集详情（Web 签名）。
pub async fn sheet_collection_detail(state: &AppState, session: &KgSession, collection_id: &str, page: i64) -> AppResult<Value> {
...
/// sheet/detail —— 曲谱详情（Default 签名，miniyueku host）。
pub async fn sheet_detail(state: &AppState, session: &KgSession, id: &str, source: &str) -> AppResult<Value> {
...
/// sheet/hot —— 热门曲谱（Web 签名）。
pub async fn sheet_hot(state: &AppState, session: &KgSession, opern_type: i64) -> AppResult<Value> {
...
/// sheet/list —— 歌曲的曲谱列表（Default 签名）。
pub async fn sheet_list(state: &AppState, session: &KgSession, album_audio_id: &str, opern_type: i64, page: i64, pagesize: i64) -> AppResult<Value> {
...
pub async fn playlist_tags(state: &AppState, session: &KgSession) -> AppResult<Value> {
...
/// playlist/track/all —— 歌单曲目（按 global_collection_id，Default 签名）。
/// 注意 begin_idx 是起始索引不是页码。
pub async fn playlist_tracks(state: &AppState, session: &KgSession, playlist_id: &str, begin_idx: i64, pagesize: i64) -> AppResult<Value> {
...
/// playlist/track/all/new —— 歌单曲目（按 listid，软依赖登录态，Default 签名）。
pub async fn playlist_tracks_new(state: &AppState, session: &KgSession, list_id: &str, page: i64, pagesize: i64) -> AppResult<Value> {
    let body = json!({
...
/// playlist/similar —— 相似歌单（body 内嵌 key，Default 签名）。
pub async fn playlist_similar(state: &AppState, session: &KgSession, ids: &str) -> AppResult<Value> {
...
/// playlist/effect —— 音效歌单（Default 签名）。
pub async fn playlist_effect(state: &AppState, session: &KgSession, page: i64, pagesize: i64) -> AppResult<Value> {
