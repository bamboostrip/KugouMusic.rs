//! 外部歌单解析业务层 —— 对应 .NET `ExternalPlaylistParser` + Netease/QQ 两个策略。
//!
//! 从网易云/QQ音乐的分享链接解析出歌单名 + 歌曲名列表（不落酷狗，纯外部抓取）。
//! QQ 用一套自定义签名（zzb 前缀），网易云走标准 API。
...
/// 解析结果（与 .NET ExternalPlaylistParseResult 一致）。
...
/// playlist/external/parse —— 解析外部歌单链接。
pub async fn parse(state: &AppState, source_text: &str) -> AppResult<ExternalPlaylistResult> {
...
mod netease {
    ...
    pub async fn parse(state: &AppState, uri: url::Url) -> ExternalPlaylistResult {
        // 短链解析
        let resolved = resolve_short_link(state, uri.clone()).await;
        ...
        // 优先用 trackIds 拿歌名；否则用 tracks 兜底
    ...
    async fn resolve_short_link(state: &AppState, uri: url::Url) -> url::Url {
        if uri.host_str().map(|h| h.eq_ignore_ascii_case("163cn.tv")).unwrap_or(false)
            && let Ok(resp) = state.http.get(uri.as_str()).send().await
            && let Ok(loc) = resp.url().as_str().parse()
        {
            return loc;
        }
        // 解析失败则原样返回
...
mod qq {
    ...
    /// QQ 签名算法（zzb 前缀）。1:1 对应 .NET BuildQqSign。
    fn build_qq_sign(param: &str) -> String {
        ...
        // .NET SelectChars 用索引（不区分大小写但输入是大写）
