...
pub async fn send(client: &Client, session: &KgSession, req: &KgRequest) -> AppResult<serde_json::Value> {
    ...
    // ===== 步骤 4：重建 body 作为签名输入 =====
    // .NET 逻辑：Body(对象)→JSON；RawBody(字符串)→直接用；BinaryBody→base64。
    ...
    // User-Agent（若调用方已在 custom_headers 里设过则跳过，否则用默认）
    ...
    // 解析 JSON；非 JSON 则回退为 {"__raw_base64__": "..."}（对应 .NET 的回退）
    ...
    Ok(match api_response::parse(root) {
        ...
        api_response::ParsedResponse::Failure { status, err_code, root } => {
            // 失败也把原始 root 返回给上层，由调用方决定要不要按 FromKgStatus 报错。
            // 这里记一条 warn，方便排查。
            tracing::warn!(
                path = %req.path,
                status = ?status,
