...
pub fn calc_new_mid(guid: &str) -> String {
    ...
    if md5_hex.is_empty() {
        // guid 为空时 md5_str 返回空，与 .NET 一致地兜底（.NET 实际不会传空 guid）
        return "0".to_string();
    }
...
fn rewrap_pem(pem: &str) -> String {
    ...
    // body = 去掉首尾（header/footer）后所有行拼接
...
pub fn rsa_encrypt_no_padding(data: &str, is_lite: bool) -> String {
    ...
    // 右填充 0 到 key_size（数据在 [0..len)，0 在 [len..key_size)）
    ...
    let c = m.modpow(&e, &n);
    let res_bytes = c.to_bytes_be(); // 大端
...
/// KGCrypto.RsaEncryptPkcs1 —— 标准 PKCS#1 v1.5（用 OAEP=Pkcs1 等价；这里用 Oaep 是 rsa crate 的封装）。
///
/// 设备注册/歌单删除的 `p` 参数用这个。输出小写 hex（调用方 .to_uppercase()）。
///
/// 注意：.NET 用的是 RSAEncryptionPadding.Pkcs1（PKCS#1 v1.5），rsa crate 的
/// `Pkcs1` 需 `sha1` 哈希；这里用 `rsa::Pkcs1v15Encrypt` 精确对齐。
...
mod crypto_tests {
    ...
    fn aes256_roundtrip_with_random_key() {
        ...
        // tempKey 是 16 字符明文，aes_decrypt 用它反推 key/iv
    ...
    fn aes256_explicit_key_produces_valid_hex() {
        // .NET 的 AesEncrypt(显式 key) 与 AesDecrypt(对 key 做 md5) 是**非对称**设计：
        // 显式 key 用于加密（原始字节），解密侧总是 md5(tempKey)。
        // 所以这里不验证往返，只验证加密产出合法 hex（与 .NET 加密侧一致）。
        ...
        assert!(enc.cipher_text.len().is_multiple_of(32)); // AES 块 = 16 字节 = 32 hex
    }
